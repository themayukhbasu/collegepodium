...
<head>
	...
	<?php
		...
		$stmt = $connection->prepare("SELECT COUNT(*) FROM feedback where state = 0");
		$stmt->bind_result($feedbackCnt);
		$stmt->execute();
		$stmt->fetch();
		$stmt->close();
		
		$stmt = $connection->prepare("SELECT COUNT(*) FROM report where state = 0");
		$stmt->bind_result($reportCnt);
		$stmt->execute();
		$stmt->fetch();
		$stmt->close();
	?>
	...
	<div class="container-fluid">
        ...
        <!-- /#sidebar-wrapper -->
			<?php 
				...
				echo 'No. of feedbacks to deal with: '.$feedbackCnt;
				echo '<br/><br/>No. of reports to deal with: '.$reportCnt;
