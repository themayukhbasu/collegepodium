...
<head>
	...
	$(document).ready( function () {
		...
		$('#upload_inp').change( function() {
			$('.glyphicon-cloud-upload').css({'color':'#33CC66', 'border':'#33CC66 2px solid'});
		});
	});
	</script>
	<?php
	...
	$type = $_GET['type'];
	$type = htmlspecialchars ( stripslashes ( $type ) );
	if ( strtolower( $type ) === 'discuss' ) {
		$type = 'discuss';
		$header = 'Start a discussion';
	}
	elseif ( strtolower( $type )  === 'query' ) {
		$type = 'query';
		$header = 'Ask a question';
	}
	elseif ( strtolower($type) === 'notice' ) {
		$type = 'notice';
		$header = 'Post a notice';
	}
            ...
            <ul class="sidebar-nav">
				...
				if(strtolower( $type ) === 'discuss')
				{
					echo '	<li>
							...
							<li>
								<a href="static_upload.php?type=discuss" class="thispage">start a discussion</a>
							</li>';
				}
				elseif ( strtolower( $type )  === 'query' )
				{
					echo '	<li>
							...
							<li>
								<a href="static_upload.php?type=query" class="thispage">post a query</a>
							</li>
							<li>
				...
				{
					echo '	<li>
								<a href="static_upload.php?type=notice" class="thispage">post a notice</a>
							</li>
							<li>
								<a href="static_upload.php?type=query">post a query</a>
							</li>
							...
							<form action="post.php" method="POST" enctype="multipart/form-data" id="uploadimage"> 
								...
								<div id="upload-form-main" class="page-content">			
									<div class="form-group">
										...
										<input type="text" name="post_title"  class="same-line" maxlength="100" required></input>
									</div class="form-group">
									<div>
										<span class="same-line"><label class="text-top">Post :</label></span> 
										<textarea name="post_data" class="same-line" required></textarea>
									
									</div>

									<div class="form-group">
										<span><label>Upload</span></label>
										<span class="upload-sign" id="upload-button" title="If green, file has been selected"><span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span></span>
