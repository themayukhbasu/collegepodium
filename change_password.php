...
<head>
	...
	$(document).ready( function () {
		...
		/*$('#new-pass').input(function() {
			if ( $(this).val() === $('#verify-new').val() ) {
				$(this).closest('.form-group').removeClass('has-error').addClass('has-success');
				$('#verify-new').closest('.form-group').removeClass('has-error').addClass('has-success');
			}
			else {
				$(this).closest('.form-group').removeClass('has-success').addClass('has-error');
				$('#verify-new').closest('.form-group').removeClass('has-success').addClass('has-error');
			}
		});
		
		$('#verify-new').input(function() {
			if ( $(this).val() === $('#new-pass').val() ) {
				$(this).closest('.form-group').removeClass('has-error').addClass('has-success');
				$('#new-pass').closest('.form-group').removeClass('has-error').addClass('has-success');
			}
			else {
				$(this).closest('.form-group').removeClass('has-success').addClass('has-error');
				$('#new-pass').closest('.form-group').removeClass('has-success').addClass('has-error');
			}
		});*/
		
		var callback = parse( 'callback' );
		if ( callback === 'wrongpass' )
			$('.upload-error').prepend('<div class="alert alert-danger inline" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>Wrong password entered.</div>');
			...
			$('.upload-error').prepend('<div class="alert alert-danger inline" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>Could not change password. Please try again.</div>');
		}
                ...
                <div class="row">
                    <div class="col-lg-12">
						<div id="upload-form-wrap" class="col-md-10">
							...
							<form action="api-passchange.php" method="POST" enctype="multipart/form-data" id="uploadimage"> 
								<h1>change password</h1>
								<div id="upload-form-main" class="page-content">			
									<div class="form-group">
										<span class="same-line"><label>Current password: </label></span>
										<input type="password" class="same-line" name="current_pass" id="old-pass" placeholder="Enter Your Current Password">
									</div>
									
									<div class="form-group">
										<span class="same-line"><label>New password: </label></span>
										<input type="password" class="same-line" name="new_pass" id="new-pass" placeholder="Enter New Password">
									</div>
									
									<!--<div class="form-group">
										<span class="same-line"><label>Confirm new password: </label></span>
										<input type="password" class="same-line" id="verify-new" name="new_pass" placeholder="Retype New Password">
									</div>-->
